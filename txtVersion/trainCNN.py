...
def evaluate_TDdata(learning_rate=0.05, n_epochs=200,
    ...
    layer2 = HiddenLayer(
        ...
        input=layer2_input,
        n_in=nkerns[1] * 4 * 4,
        n_out=500,  # 全连接层输出神经元的个数，自己定义的，可以根据需要调节
        activation=T.tanh
    )

    # 分类器
    layer3 = LogisticRegression(input=layer2.output, n_in=500, n_out=3)  # n_in等于全连接层的输出，n_out等于40个类别
