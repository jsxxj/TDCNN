...
def test_CNN(dataset='./testData/testData4.pkl', params_file='Dropout_2_2_CNN_params.pkl'):
    dataset = './testData/testData4.pkl'
    ...
    for i in range(len(test_set_x)):
        ...
        if float(pred[i]) == float(label_y[i]):
            ...
            print('the data: %i is class %i, but mis-predicted as class %i' % (i, label_y[i], pred[i]))

    print 'the predicted correct samples is %',count
