# -*- coding: utf-8 -*-
"""
Created on Thu Nov 23 11:32:09 2017
...
def loadData():
    ...
    data = np.loadtxt('dataLabel.txt')
    timeData =np.asarray(data,dtype='float64')

    np.random.shuffle(timeData) # 打乱时序数据顺序
    
    valid_boundary = int(timeData.shape[0]*0.8)
    test_boundary = int(timeData.shape[0]*0.9)
    '''
    划分数据集
    '''
    x = timeData[:,:-1]/255
    ...
    train_data = x[:valid_boundary]
    train_label = y[:valid_boundary]
    valid_data = x[valid_boundary:test_boundary]
    valid_label = y[valid_boundary:test_boundary]
    test_data = x[test_boundary:]
    test_label = y[test_boundary:]
...
class LogisticRegression(object):
    def __init__(self, input, n_in, n_out):
        ...
        # self.p_value = T.argmax(self.p_y_given_x)
...
def evaluate_TDdata(learning_rate=0.05, n_epochs=200,
                    nkerns=[5, 10], batch_size=60):
    # 随机数生成器，用于初始化参数
    ...
    layer0_input = x.reshape((batch_size, 1, 36, 40))
    ...
    layer0 = LeNetConvPoolLayer(
        rng,
        input=layer0_input,
        image_shape=(batch_size, 1, 36, 40),
    ...
    layer1 = LeNetConvPoolLayer(
        rng,
        input=layer0.output,
        image_shape=(batch_size, nkerns[0], 16, 18),
    ...
    # 分类器
    layer3 = LogisticRegression(input=layer2.output, n_in=2000, n_out=3)  # n_in等于全连接层的输出，n_out等于40个类别
    ...
    print('Best validation score of %f %% obtained at iteration %i, '
          ...
          (100 - best_validation_loss * 100., best_iter + 1, 100 - test_score * 100.))
